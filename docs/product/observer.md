...
事项详情页聚合报名提交、准入提案建立、开始表决、实名票据、确定性判定、执行结果和守约者任命事件，并提供审计证明。公开投影只展示脱敏报名者标签、提案阶段、票数和公开决定，不展示联系方式、报名私密字段、内部选民选择器或 OpenFGA 标识。审计记录 seq 表示事实顺序，Hash 复算和 json_script 安全机制与普通事件详情页一致。
