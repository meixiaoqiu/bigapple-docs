...
积分发行是独立的治理动作，只负责把积分放入公共发行池。任务发布时，系统按“基础积分 × 岗位系数”计算预计奖励，并自动从发行池补锁当前缺口；已有部分或足额预算时只补差额或不重复锁定。发行池余额不足时不自动发行、也不部分锁定，任务保留为草稿并显示预计奖励、已锁预算、池余额和差额。积分预算页继续提供人工追加锁定、退回未用预算等维护能力，并以可识别的任务列表代替复制任务 ID。表单使用 per-render `idempotency_key` 防重复提交。
...
- 创建页提供“保存草稿”和“锁定预算并发布”两个明确动作；后者把创建、补锁缺口和发布作为一个原子操作。
- `base_points=0` 表示无积分奖励任务，可直接发布，不需要锁定预算。
- `base_points>0` 的任务发布时自动补足锁定预算；发行池不足时保留草稿，不产生部分锁定或发布事件。
