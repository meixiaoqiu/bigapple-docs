...
- 提交已领取任务的劳动记录和证据引用。
- 提交申诉。
- 从“我的事务”的最近结束任务进入任务详情，查看劳动、证据和验收记录；同时查看近期事件、资源预警和申诉状态。
...
当前处于迁移对照阶段：新的“我的事务”区域位于旧首页模块之前，原有待处理事项、统计、当前任务、可领取任务、积分流水、资源预警、相关事件和申诉状态继续保留。“下一步动作”和首页“个人任务历史”已在逐项确认后删除；结束任务改由“最近结束”进入独立详情。其他旧模块仍只会在后续逐项确认后单独删除或迁移。
