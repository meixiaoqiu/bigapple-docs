...
完整成员的 Workspace 首页使用专用紧凑页头，取代面向宽屏网站的横向公共页头。页头在同一行左侧显示现有社区品牌文字并指向首页地址，右侧是一个带"页面导航"可访问名称的折叠菜单入口；菜单默认收起，展开后按运行时导航上下文 `runtime_nav.items` 的现有顺序完整展示当前用户可见的全部入口（首页、事件流、财务、资源库存、我的主页、工作台、退出），不因窄画布删除、重排或新增任何入口。
...
该紧凑页头仅作用于完整成员首页；任务中心、财务等 Workspace 子页面、报名工作台和其他运行时页面继续使用变更前的公共运行时页头，不受影响。

### 首页顶部欢迎与状态摘要

紧凑页头之后，Workspace 首页以轻量的白色欢迎区域开场，依次展示"你好，"和"欢迎回到工作台"，不再使用早期面向旧后台风格的整块黄色英雄卡与重阴影。欢迎文案是稳定文案，不根据客户端或服务端时间切换"早上好/下午好"。

欢迎文案下方是一张细边框、无重阴影的双列状态摘要卡：左列展示 `Member` 的当前状态显示值，并以当前成员的资格或派生身份作为辅助文本；右列展示积分服务计算得到的 `credit_balance`（当前积分）与积分下限。成员编号、当前 world 标识和模拟日以紧凑元数据行保留在摘要之后，职责仍按既有身份数据循环展示，不写死展示值，也不丢失任何运行上下文。

成员状态和当前积分从页面后部的核心状态区上移到顶部摘要，属于信息位置迁移而非删除指标；后部核心状态区继续保留可用积分与历史贡献，避免同一指标在首页重复出现。页面不使用设计稿中的示例积分、通知按钮或底部导航。
