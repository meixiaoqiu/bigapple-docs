...
有效守约者可从工作台进入 `/workspace/deliberator-exam/`。系统按当前政策从已发布题库随机抽题，只接受当前登录成员自己的答卷。结果页仅显示得分、总分、及格线和任期结果，不显示正确答案、解析或评分快照。未通过后可重新参加；已有有效任期时不能重复申请。
...
注册成员可以在 workspace 中提交自己的报销申请，并查看自己的报销状态。拥有 `finance.review`、`finance.pay` 或 `finance.publish_public_attachments` 权限的财务成员可以按职责查看全部报销；普通成员不能查看他人的报销详情。公开副本发布按钮和服务只对 `finance.publish_public_attachments` 开放，审核、付款和私有查看权限不隐含发布能力。

财务审核和付款不是用户自助资料的一部分，必须由财务角色执行。申请人不能审核或付款自己的报销；拒绝报销必须填写理由。已批准并付款的记录会生成只追加 `FinanceTransaction` 流水，并进入 `/finance/` 公开财务页。

具备角色维护权限的典守者可访问 `/workspace/finance/reviewer-appointments/`，从当前有效守约者中提名财务审核者。页面展示待表决、可执行提案和当前审核者；提名者不能选择任意角色，服务端固定为财务审核者。只有提案快照中当前仍合格的执衡者可以投票，提案通过后仍需有权典守者执行才产生任命。

新提交的报销必须至少上传一份支出凭证，支持 PDF、JPEG、PNG、WebP 和 UTF-8 CSV。凭证原件默认私有，申请人和具备相应财务权限的成员通过受控下载查看；页面不暴露 OCI bucket 或 object key。付款确认必须填写付款日期、付款方式并至少上传一份付款凭证。

公开财务材料不是对私有原件设置公开开关。财务人员需要另行上传经过脱敏检查的独立副本，公开页只提供该副本。银行卡号、支付账号、联系方式、内部备注、私有文件名和原始后端响应不会公开。
