...
- 成员报名写入 `MemberApplication` 并幂等创建准入提案。显式启用仿真 bootstrap 管理员的 `zero_start` seed 会幂等发布“管理员”选民、单票阈值、168 小时期的准入测试政策，reset 后可恢复同一测试基线；未启用该配置时提案保持“等待政策配置”。仿真的候选/备用/拒绝/退出筛选结果仍只写入 `metadata.screening_status`，不直接改写权威准入结果。真实 world 不会因仿真 seed 自动发布示例政策。
...
`MemberApplication.status` 是权威报名结果（`submitted` / `admitted` / `rejected` / `withdrew`）；表决中间态只保存在关联统一提案。`metadata.screening_status` 是仿真筛选口径（`candidate` / `standby` / `rejected` / `withdrew`），两种状态机互不干扰。
