...
仿真 world 初始化时会建立一题、满分通过的默认执衡者考试政策。首位已有守约者资格的成员需实际通过该考试取得执衡者任期，随后才能表决新账号的守约者申请；seed 和角色基线不得绕过考试直接制造执衡者任期。

若成员报名发生在首位执衡者产生之前，其准入提案会得到空选民快照。首位执衡者通过考试后，系统会取消这类无法推进的旧提案并为同一报名重建提案，保留完整审计链。新成员准入后仍无财务权限，必须再经财务审核职责任命提案才能审核他人报销。
