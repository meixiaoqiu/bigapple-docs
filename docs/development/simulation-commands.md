...
`zero_start` 模板只预置一个初始发起人、一个极简计划和一个已发布计划版本，不预置任务、资源、候选场地或成熟成员池。启用 `BIG_APPLE_SIMULATION_BOOTSTRAP_MAINTAINER_ENABLED=true` 时，这个初始成员就是配置的仿真 bootstrap maintainer 登录成员；未启用时才使用非交互 fallback 发起人。
