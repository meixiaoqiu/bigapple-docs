...
账号应由管理员在 Live OS 中通过正常的产品或管理流程准备：

- **普通成员账号**：能正常登录并访问成员工作区；
- **管理员账号**：按 Live OS 正常角色与权限机制获得报名处理权限；
...
| `DOCS_SCREENSHOT_GOVERNANCE_LOGIN` | 已有管理员账号的登录标识 |
| `DOCS_SCREENSHOT_GOVERNANCE_PASSWORD` | 已有管理员账号的密码 |
...
如果管理员希望截图长期稳定，应在 Live OS 自己的开发环境中维护通用测试数据或演示世界。这类数据能力应当对 Live OS 本身有独立价值，不应为了 Docs 截图专门写入。Docs 只消费页面最终呈现结果。
