...
| 名称 | 取得方式 | 是否有期限 | 作用 |
| --- | --- | --- | --- |
| 守约者 | 守约者准入 | 由任命记录决定 | 成为执衡者或典守者的前提。 |
| 执衡者 | 有效守约者本人申请，无需审核 | 一年，到期不自动续任 | 承担执衡义务，并按提案的选民规则参与表决。 |
| 典守者 | 正常任命 | 由任命记录决定 | 仅拥有明确绑定的典守权限；不会因此成为执衡者，只能参与制度明确指定的典守事务。 |
...
| 典守事务 | 当前有效典守者；典守身份不因此产生执衡投票权。 |

典守者不是守约事务或专业事务的自动选民。成员资格、执衡者任期、专业资格、用户停用状态和当前 world 都在投票时重新校验；提案开始时的快照不能绕过之后失效的授权。
...
成功后，基线包含守约者、执衡者、典守者、具有财务专业资格的执衡者，以及必要的 OpenFGA tuple。重置会清理该仿真 world 中用于本场景的角色、任命、资格和提案选民数据；不可用于 `realworld`。
...
| 典守者 | 显示守约者和典守者；可执行明确维护操作；不可投票。 | `core.tests.test_openfga_role_policy.OpenFGARolePolicyTests`、`core.tests.test_proposals.ProposalVotingPolicyTests.test_covenanter_or_maintainer_without_deliberator_term_cannot_vote` |
| 兼任执衡者和典守者 | 两项职责同时显示；维护权和投票权分别计算。 | `core.tests.test_identity_display` |
...
界面验收时可打开 `/u/role-baseline-deliberator/` 和 `/u/role-baseline-maintainer/`：前者应显示“守约者、执衡者”，后者应显示“守约者、典守者”，且典守者页面不应显示“执衡者”。
