...
- **公开反馈不是治理决策**：`CommunityFeedback` 只用于注册用户公开提问、建议、担忧或倡议。它不得直接改变权威状态或授予权限；如需正式行动，必须等待统一提案系统承接或进入明确允许的领域服务。Feedback 不写 `SystemEvent` 哈希链，只按规则写普通公开 `Event`；隐藏反馈必须撤下既有公开 Event。
