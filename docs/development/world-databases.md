...
- 目标 world DB 的首个管理员：默认用户名和成员编号 `member-admin-0001`，`is_staff=False`，`is_superuser=False`，并拥有管理员任命和 `governance.*` 基础权限。
...
6. 用 `bootstrap_world --world-id simulation0002` 创建该仿真 world 的首个管理员。
