...
扫描拥有 `ROLE_COVENANTER` 角色任命但没有守约者编号凭证的成员，补发 `covenanter_number` Credential Grant。

```powershell
.\.venv\Scripts\python.exe manage.py repair_covenanter_credentials --world-id realworld --dry-run
.\.venv\Scripts\python.exe manage.py repair_covenanter_credentials --world-id realworld
```
...
- 非 dry-run 时才调用 `ensure_builtin_credential_templates()` 和 `issue_covenanter_number()`。
- 已有 `covenanter_number` 凭证的成员不会被重复发放。
...
- 目标 world DB 的首个典守者：默认用户名和成员编号 `member-admin-0001`，`is_staff=False`，`is_superuser=False`，并拥有典守者任命和 `governance.*` 基础权限；该职责不自动取得执衡者任期或投票权。
...
只有显式启用并同时提供用户名和密码时，`seed_world` 每次成功初始化目标仿真 world 后才会确保该账号存在并拥有典守者职责。`BIG_APPLE_SIMULATION_BOOTSTRAP_MAINTAINER_PASSWORD=CHANGE_ME` 是模板占位符，启用前必须改掉，否则命令会失败。这样重置 `simulation0001` 对应数据库后，再运行 `seed_world simulation0001 --template ...`，即可继续使用该账号登录 `bigsim.local/workspace/`。
...
6. 用 `bootstrap_world --world-id simulation0002` 创建该仿真 world 的首个典守者。
